document.addEventListener("DOMContentLoaded", function(event) {
    const $signIn = document.querySelector('.header-sup .sign-in')
    vtexjs.checkout.getOrderForm()
        .done(function(orderForm) {
            if (orderForm.loggedIn) {
                $signIn.classList.add('logged')
                if (orderForm.clientProfileData.firstName) {
                    $signIn.querySelector('.welcome-message .welcome-name').innerText = orderForm.clientProfileData.firstName
                } else if (orderForm.clientProfileData.email) {
                    $signIn.querySelector('.welcome-message .welcome-name').innerText = orderForm.clientProfileData.email
                }
            }
        })
